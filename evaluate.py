...
"""
Copy evaluate script from Chainer CV
chainercv をインストールしない理由
- バージョン管理がだるい
- 自前の実装より速かったから
- 汎用性があってステキ
etc...
...
def calc_semantic_segmentation_iou(confusion):
    ...
    * :math:`\\text{IoU of the i-th class} =  \
    ...
    iou_denominator = (confusion.sum(axis=1) + confusion.sum(axis=0)
                       - np.diag(confusion))
    # RuntimeWarning: invalid value encountered in true_divideが出た場合,ゼロ除算が発生していた可能性
...
def eval_semantic_segmentation(pred_labels, gt_labels):
    ...
    * :math:`\\text{Mean Class Accuracy} = \\frac{1}{k} \
    ...
    return {'IoU': iou,
            ...
            'MeanClassAccuracy': np.nanmean(class_accuracy)}


if __name__ == '__main__':
    pred_paths = 'predict_image/*.png'
    true_paths = 'ground_true_image/*.png'
    size = (224, 224)

    # 画像パスの取得
    pred_paths = sorted(glob.glob(pred_paths))
    true_paths = sorted(glob.glob(true_paths))
    print('path length : {}'.format(len(pred_paths)))

    # PILによる画像配列の取得
    read_img = lambda path: Image.open(path).convert('L').resize(size, Image.NEAREST)
    pred_pil_array = [read_img(path) for path in pred_paths]
    true_pil_array = [read_img(path) for path in true_paths]

    # 配列の型をPILからnumpyへ変換
    convert_pil2numpy = lambda pil_array: np.asarray(pil_array, dtype=np.int64)
    pred_numpy_array = list(map(convert_pil2numpy, pred_pil_array))
    true_numpy_array = list(map(convert_pil2numpy, true_pil_array))

    # 評価値の算出
    ess = eval_semantic_segmentation(pred_numpy_array, true_numpy_array)
    pix_acc = ess['PixelAccuracy'].round(5) * 100.
    mean_cls_acc = ess['MeanClassAccuracy'].round(5) * 100.
    mean_iou = ess['MeanIoU'].round(5) * 100.
    print('Pixel Accuracy      :{:>7} %'.format(pix_acc))
    print('Mean Class Accuracy :{:>7} %'.format(mean_cls_acc))
    print('mean IoU            :{:>7} %'.format(mean_iou))
